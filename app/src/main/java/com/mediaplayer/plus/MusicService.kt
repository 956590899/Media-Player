...
class MusicService : Service() {
    ...
    private fun updateMetadata(title: String, subtitle: String, duration: Long, albumArt: Bitmap?) {
        ...
        if (albumArt != null && !albumArt.isRecycled) {
            // METADATA_KEY_ALBUM_ART：主字段，AVRCP 1.6 封面图传输用
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART, albumArt)
            // METADATA_KEY_ART：备用字段，部分 ROM 依赖此字段读取封面
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ART, albumArt)
            // METADATA_KEY_DISPLAY_ICON：通知栏/锁屏显示图标
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_DISPLAY_ICON, albumArt)
        } else {
            // MIUI Dynamic Island 必须检测到 METADATA_KEY_ALBUM_ART 才能正常渲染
            // 缺失时 DynamicIslandBackgroundView 高度=0 并触发 checkError
            val placeholder = android.graphics.Bitmap.createBitmap(1, 1, android.graphics.Bitmap.Config.ARGB_8888)
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ALBUM_ART, placeholder)
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_ART, placeholder)
            metadataBuilder.putBitmap(MediaMetadataCompat.METADATA_KEY_DISPLAY_ICON, placeholder)
        }
